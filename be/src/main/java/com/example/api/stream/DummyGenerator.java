...
public class DummyGenerator {
    ...
    public Student makeStudent() {
        return new Student(makeName(), makeUsername(),
                RefUtil.rangeBelowRandom.apply(1, 3), RefUtil.rangeBelowRandom.apply(0, 300));
    }

    public List<Student> makeStudentList(int n) {
        List<Student> list = new ArrayList<>();

        for (int i = 0; i < n; i++) {
            list.add(makeStudent());
        }
        return list;
    }
    ...
    private String myCalendar(int start, int end) {
        int year = RefUtil.rangeBelowRandom.apply(start, end);
        int month = RefUtil.rangeBelowRandom.apply(1, 12);
        int date30 = RefUtil.rangeBelowRandom.apply(1, 30);
        int date31 = RefUtil.rangeBelowRandom.apply(1, 31);
        int date29 = RefUtil.rangeBelowRandom.apply(1, 29);
        int date28 = RefUtil.rangeBelowRandom.apply(1, 28);
        int date = 0;
        switch (month) {
            case 4:
            case 6:
